...
function Welcome() {
  ...
  const handleRunpodSetup = async () => {
    try {
      ...
      await initializeRunpodSettings(settings.runpodApiKey, settings.runpodInstanceId);
      refreshSettings();
      navigate("/");
    } catch (error) {
  ...
  return (
    <main className="w-screen h-screen bg-background text-foreground flex flex-col items-center justify-center">
      ...
      <h3 className="text-2xl text-muted-foreground animate-in fade-in-0 duration-300">Your new home for cooking up memes</h3>
