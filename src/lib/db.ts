...
class DatabaseQueue {
  ...
  private currentTransaction: Promise<void> | null = null;
  ...
  private async processQueue() {
    ...
    if (this.currentTransaction) {
      try {
        ...
        console.error('Previous transaction failed:', error);
      }
    }
    ...
    this.currentTransaction = (async () => {
      try {
        ...
        item.resolve(result);
        this.queue.shift(); // Remove the completed operation
      } catch (error) {
        ...
        if (item.retries < this.maxRetries) {
          item.retries++;
          console.log(
            `Retrying operation (attempt ${item.retries}/${this.maxRetries})...`
          );
          ...
          item.reject(error);
          this.queue.shift(); // Remove the failed operation
        }
      }
    })();
